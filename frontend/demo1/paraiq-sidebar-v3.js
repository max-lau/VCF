...
(function () {
  ...
  function logout() {
    if (window.ParaIQ && ParaIQ.logout) {
      ParaIQ.logout();
    } else {
      try {
  ...
  var GROUPS = [
    ...
    },
    {
      ...
      items: [
        ...
        { href: 'media.html', label: 'Media & Messaging', icon: '🎙' },
        { href: 'cases.html', label: 'Case Management', icon: '⚖️' },
        { href: 'intelligence.html', label: 'AI Intelligence', icon: '🧠' },
        { href: 'admin.html', label: 'Admin', icon: '⚙️', role: 'admin' },
  ...
  function buildNav() {
    ...
    for (var g = 0; g < GROUPS.length; g++) {
      ...
      var _role = (window.ParaIQ && ParaIQ.getRole) ? ParaIQ.getRole() : 'user';
      var _hier = { 'user': 1, 'manager': 2, 'admin': 3 };
      var _level = _hier[_role] || 1;
      for (var i = 0; i < group.items.length; i++) {
        var item = group.items[i];
        if (item.role && (_hier[item.role] || 1) > _level) continue;
      ...
      '<div class="sb-footer">' +
        '<div class="sb-user">' +
          ...
          '<div class="sb-user-info">' +
            ...
            '<div class="sb-user-role">'+(window.ParaIQ&&ParaIQ.getRole?ParaIQ.getRole():'user')+'</div>' +
          '</div>' +
          '<button class="sb-logout" title="Sign out" onclick="(function(){' +
            'if(window.ParaIQ&&ParaIQ.logout){ParaIQ.logout();}else{try{Object.keys(localStorage).filter(function(k){return k.indexOf(\'paraiq_\')==0;}).forEach(function(k){localStorage.removeItem(k);});}catch(e){}window.location.href=\'login.html\';}' +
            'window.location.href=\'login.html\';' +
          '})()">⏻</button>' +
        '</div>' +

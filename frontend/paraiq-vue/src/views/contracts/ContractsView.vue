...
const form = ref({
  case_number:'', contract_name:'', contract_type:'', parties:'',
  execution_date:'', expiry_date:'', status:'draft', notes:''
})
...
async function fetchAll() {
  ...
  try {
    const [iRes, sRes] = await Promise.all([
      ...
      client.get('/contracts/stats'),
    ])
    items.value = iRes.data.contracts || []
...
async function save() {
  ...
  saving.value  = true
  try {
    await client.post('/contracts/', form.value)
    showForm.value = false
    form.value = { case_number:'', contract_name:'', contract_type:'', parties:'', execution_date:'', expiry_date:'', status:'draft', notes:'' }
    await fetchAll()
  } catch(e) {
...
async function uploadForContract(item) {
  try {
    ...
    uploadMatterName.value = m?.client_name || item.contract_name
    uploadCaseNum.value    = m?.case_number || item.case_number
  } catch {
    uploadMatterId.value   = 0
    uploadMatterName.value = item.contract_name
...
function statusColor(s) {
  return {
    draft:'#718096', active:'#48bb78', expired:'#fc8181', terminated:'#718096'
  }[s] || '#a0aec0'
}
...
<template>
  <div class="mod">
    ...
    <div class="mod__header">
      <div>
        <h1 class="mod__title">Contracts</h1>
        <p class="mod__sub">Agreements, retainers, and exhibits under review</p>
      </div>
      <div class="header-actions">
        <button class="btn-upload" @click="openPicker">↑ Upload Contract</button>
        <button class="piq-btn-gold" @click="showForm = !showForm">+ New Contract</button>
      </div>
    </div>
    ...
    <div v-if="showForm" class="form-card">
      <h3 class="form-card__title">New Contract</h3>
      <div class="form-grid">
        <label>Case Number<input v-model="form.case_number" class="piq-input" placeholder="2026-CV-00142"/></label>
        <label>Contract Name<input v-model="form.contract_name" class="piq-input" placeholder="Employment Agreement"/></label>
        <label>Type
          <select v-model="form.contract_type" class="piq-input">
        ...
        <label>Parties<input v-model="form.parties" class="piq-input" placeholder="Smith, Acme Corp"/></label>
        <label>Execution Date<input v-model="form.execution_date" class="piq-input" type="date"/></label>
        <label>Expiry Date<input v-model="form.expiry_date" class="piq-input" type="date"/></label>
        <label>Status
    ...
    <div v-else-if="!filtered.length" class="empty-state">
      ...
      <div class="empty-state__title">No contracts yet</div>
      <div class="empty-state__sub">Click "+ New Contract" to add the first one.</div>
    </div>
    ...
    <div v-else class="piq-table-wrap">
      <table class="piq-table">
        <thead>
          <tr>
            <th>ID</th><th>Name</th><th>Case</th><th>Type</th>
            <th>Parties</th><th>Executed</th><th>Expires</th><th>Status</th><th>File</th><th></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="item in filtered" :key="item.id">
            <td class="dim mono">{{ item.id }}</td>
            <td class="bold">{{ item.contract_name }}</td>
            <td class="dim mono">{{ item.case_number }}</td>
            <td class="dim">{{ item.contract_type || '—' }}</td>
            <td class="dim">{{ item.parties || '—' }}</td>
            <td class="dim nowrap">{{ fmtDate(item.execution_date) }}</td>
            <td class="dim nowrap">{{ fmtDate(item.expiry_date) }}</td>
            <td>
            ...
            <td>
              <button class="action-btn" @click="uploadForContract(item)">↑ Upload</button>
            </td>
            <td>
        ...
        <div class="picker-modal">
          <div class="picker-header">
            <div>
              ...
              <p class="picker-sub">Upload contract for which matter?</p>
            </div>

...
const form = ref({
  case_number:'', witness_name:'', witness_role:'',
  depo_date:'', location:'', status:'scheduled', notes:''
})
...
async function fetchAll() {
  ...
  try {
    const [iRes, sRes] = await Promise.all([
      client.get('/depositions/'),
      client.get('/depositions/stats'),
    ])
    items.value = iRes.data.depositions || []
...
async function save() {
  ...
  saving.value  = true
  try {
    await client.post('/depositions/', form.value)
    showForm.value = false
    form.value = { case_number:'', witness_name:'', witness_role:'', depo_date:'', location:'', status:'scheduled', notes:'' }
    await fetchAll()
  } catch(e) {
...
async function remove(id) {
  ...
  await client.delete(`/depositions/${id}`)
  await fetchAll()
}
...
function selectMatter(m) {
  ...
  uploadMatterName.value = m.client_name || m.name || `Matter #${m.id}`
  uploadCaseNum.value    = m.case_number || ''
  showPicker.value       = false
  showUpload.value       = true
}

// Pre-scope upload to a specific deposition's case number
async function uploadForDepo(item) {
  mattersLoading.value = true
  try {
    ...
    if (m) {
      ...
      uploadMatterName.value = item.witness_name
...
function statusColor(s) {
  return {
    draft:'#718096', scheduled:'#4a7cf7', completed:'#48bb78',
    filed:'#9f7aea', pending:'#ecc94b', cancelled:'#fc8181'
  }[s] || '#a0aec0'
}
...
<template>
  <div class="mod">
    ...
    <div class="mod__header">
      <div>
        <h1 class="mod__title">Depositions</h1>
        <p class="mod__sub">Witness depositions and testimony prep</p>
      </div>
      <div class="header-actions">
        <button class="btn-upload" @click="openPicker">↑ Upload Transcript</button>
        <button class="piq-btn-gold" @click="showForm = !showForm">+ New Deposition</button>
      </div>
    </div>
    ...
    <div v-if="showForm" class="form-card">
      <h3 class="form-card__title">New Deposition</h3>
      <div class="form-grid">
        <label>Case Number<input v-model="form.case_number" class="piq-input" placeholder="2026-CV-00142"/></label>
        <label>Witness Name<input v-model="form.witness_name" class="piq-input" placeholder="John Smith"/></label>
        <label>Witness Role<input v-model="form.witness_role" class="piq-input" placeholder="Plaintiff, Expert, etc."/></label>
        <label>Deposition Date<input v-model="form.depo_date" class="piq-input" type="date"/></label>
        <label>Location<input v-model="form.location" class="piq-input" placeholder="Office or remote"/></label>
        <label>Status
          <select v-model="form.status" class="piq-input">
            <option>scheduled</option><option>completed</option><option>cancelled</option>
          </select>
        </label>
        <label style="grid-column:1/-1">Notes<input v-model="form.notes" class="piq-input" placeholder="Prep notes…"/></label>
      </div>
    ...
    <div v-else-if="!filtered.length" class="empty-state">
      ...
      <div class="empty-state__title">No depositions yet</div>
      <div class="empty-state__sub">Click "+ New Deposition" to add the first one.</div>
    </div>
    ...
    <div v-else class="piq-table-wrap">
      <table class="piq-table">
        <thead>
          <tr>
            <th>ID</th><th>Witness</th><th>Case</th><th>Role</th>
            <th>Date</th><th>Location</th><th>Status</th><th>Transcript</th><th></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="item in filtered" :key="item.id">
            <td class="dim mono">{{ item.id }}</td>
            <td class="bold">{{ item.witness_name }}</td>
            <td class="dim mono">{{ item.case_number }}</td>
            <td class="dim">{{ item.witness_role || '—' }}</td>
            <td class="dim">{{ fmtDate(item.depo_date) }}</td>
            <td class="dim">{{ item.location || '—' }}</td>
            <td>
            ...
            <td>
              <button class="action-btn" @click="uploadForDepo(item)">↑ Upload</button>
            </td>
            <td>
        ...
        <div class="picker-modal">
          <div class="picker-header">
            <div>
              ...
              <p class="picker-sub">Upload transcript for which matter?</p>
            </div>

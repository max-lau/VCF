    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        for cfg in configs:
            try:
                ...
                log_delivery(platform, event_type, resp.status_code, success, preview=str(payload)[:100])
...
async def send_risk_alert(body: SendRiskAlert):
    ...
    slack_result = await send_to_platform("slack", build_slack_risk(risk, body.label), "risk_alert", mock=body.mock)
    teams_result = await send_to_platform("teams", build_teams_risk(risk, body.label), "risk_alert", mock=body.mock)

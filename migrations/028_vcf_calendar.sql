...
-- Make matter_id nullable if the column exists (some deployments never had it).
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'calendar_events' AND column_name = 'matter_id'
    ) THEN
